#ifndef CPP_BASICEKF_H
#define CPP_BASICEKF_H

#include "base_struct.h"
#include "CtrlStates.h"
#include "Utils.h"

// state estimator parameters
#define STATE_SIZE 18
#define MEAS_SIZE 28
#define PROCESS_NOISE_PIMU 0.01
#define PROCESS_NOISE_VIMU 0.01
#define PROCESS_NOISE_PFOOT 0.01
#define SENSOR_NOISE_PIMU_REL_FOOT 0.001
#define SENSOR_NOISE_VIMU_REL_FOOT 0.1
#define SENSOR_NOISE_ZFOOT 0.001

// implement a basic error state KF to estimate robot pose
// assume orientation is known from a IMU (state.root_rot_mat)
class BasicEKF {
public:
    BasicEKF ();
    BasicEKF (bool assume_flat_ground_);
    void init_state(CtrlStates& state);
    void update_estimation(CtrlStates& state, double dt);
    bool is_inited() {return filter_initialized;}
private:
    bool filter_initialized = false;
    // state
    // 0 1 2 pos 3 4 5 vel 6 7 8 foot pos FL 9 10 11 foot pos FR 12 13 14 foot pos RL 15 16 17 foot pos RR
    // estimation state
    Eigen::Matrix<double, STATE_SIZE, 1> x; 
    // estimation state after process update
    Eigen::Matrix<double, STATE_SIZE, 1> xbar; 
    // estimation state covariance
    Eigen::Matrix<double, STATE_SIZE, STATE_SIZE> P; 
    // estimation state covariance after process update
    Eigen::Matrix<double, STATE_SIZE, STATE_SIZE> Pbar; 
    // estimation state transition
    // I I*dt  0 0 0 0
    // 0 I     0 0 0 0
    // 0 0     I 0 0 0
    // 0 0     0 I 0 0
    // 0 0     0 0 I 0
    // 0 0     0 0 0 I
    Eigen::Matrix<double, STATE_SIZE, STATE_SIZE> A; 
    // estimation state transition
    // 0
    // I*dt
    // 0
    // 0
    // 0
    // 0
    Eigen::Matrix<double, STATE_SIZE, 3> B; 
    //估计误差
    Eigen::Matrix<double, STATE_SIZE, STATE_SIZE> Q; // estimation state transition noise

    // observation
    // 0 1 2   FL pos residual
    // 3 4 5   FR pos residual
    // 6 7 8   RL pos residual
    // 9 10 11 RR pos residual
    // 12 13 14 vel residual from FL
    // 15 16 17 vel residual from FR
    // 18 19 20 vel residual from RL
    // 21 22 23 vel residual from RR
    // 24 25 26 27 foot height
    Eigen::Matrix<double, MEAS_SIZE, 1> y; 
    // estimated observation
    Eigen::Matrix<double, MEAS_SIZE, 1> yhat; 
    // estimated observation
    Eigen::Matrix<double, MEAS_SIZE, 1> error_y; 
    // S^-1*error_y
    Eigen::Matrix<double, MEAS_SIZE, 1> Serror_y; 
    // estimation state observation
    Eigen::Matrix<double, MEAS_SIZE, STATE_SIZE> C; 
    // S^-1*C
    Eigen::Matrix<double, MEAS_SIZE, STATE_SIZE> SC; 
    // estimation state observation noise 量测误差
    Eigen::Matrix<double, MEAS_SIZE, MEAS_SIZE> R; 

    // helper matrices

    // 3x3 identity
    Eigen::Matrix<double, 3, 3> eye3; 
    // Innovation (or pre-fit residual) covariance
    Eigen::Matrix<double, MEAS_SIZE, MEAS_SIZE> S;
    // kalman gain  
    Eigen::Matrix<double, STATE_SIZE, MEAS_SIZE> K; 


    bool assume_flat_ground = false;

    // variables to process foot force
    double smooth_foot_force[4];
    //表示第 i 条腿的估计接触状态。这个状态值在 0 到 1 之间，0 表示没有接触，1 表示完全接触。
    double estimated_contacts[4];
};


#endif //A1_CPP_A1BASICEKF_H
